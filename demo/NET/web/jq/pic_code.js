...
var pic_code = {
    ...
    init: function(opt) {
        ...
        var t_opt = {
            ...
            Callback_success_apierror: null // 验证成功后，请求接口发生服务级错误，无默认
        };
    ...
    //验证失败小块滑回原位
    // error_msg：默认 拖动滑块，完成正确拼图 || 系统将自动刷新验证码
    doMove: function(error_msg) {
        //取消滑块滑动事件
        pic_code.Cancle_oCircle_Click();
        //隐藏刷新按钮
        ...
        if (pic_code.pic_code_error_count.error < pic_code._opt.Callback_error_repeatedly_count) {
            error_msg = error_msg || "拖动滑块，完成正确拼图";
        } else {
            error_msg = error_msg || "系统将自动刷新验证码";
        }
            pic_code.pic_fail.html('<span style="color:#ff0000">验证失败</span> : ' + error_msg);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            switch (data.error.toLowerCase()) {
                                case "success": // 成功
                                    pic_code._opt.Callback_success(data.result[0]);
                                    break;
                                case "apierror": // 验证成功后，接口报错
                                    if (pic_code._opt.Callback_success_apierror)
                                        pic_code._opt.Callback_success_apierror(data.error_msg);
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var reset = function(msg) {
                                pic_code.pic_code_error_count.error += 1;
                                pic_code.pic_code_circle.unbind('mousedown touchstart');
                                pic_code._opt.Callback_error(msg);
                                if (pic_code.pic_code_error_count.error >= pic_code._opt.Callback_error_repeatedly_count) {
                                    setTimeout(function() {
                                        pic_code._opt.Callback_error_repeatedly();
                                    }, 500);
