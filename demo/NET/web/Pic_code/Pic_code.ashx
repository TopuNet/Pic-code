<%@ WebHandler Language="C#" Class="Pic_code" %>
...
public class Pic_code : IHttpHandler,IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (!di1.Exists)
        {
            context.Response.Write("{\"error\":\"ERROR\",\"msg\":\"验证码底图库不存在：" + captchapath + "\"}");
        ...
        if (fi.Length == 0)
        {
            context.Response.Write("{\"error\":\"ERROR\",\"msg\":\"验证码底图不存在：" + captchapath + "\"}");
            context.Response.End();
        }
        #endregion
    ...
    /// <returns></returns>
    public static string Make_Pic_code(string FromFilePath, string saveFilePath1, string saveFilePath2)
    {
        #region ==验证底图和水印图是否存在==
        if (!File.Exists(FromFilePath))
        {
            return "{\"error\":\"ERROR\",\"msg\":\"底图不存在：" + FromFilePath + "\"}";
        }

        string waterFilePath = HttpContext.Current.Server.MapPath(captchapath + "water.png");
        if (!File.Exists(waterFilePath))
        {
            return "{\"error\":\"ERROR\",\"msg\":\"水印图不存在：" + waterFilePath + "\"}";
        }
        #endregion
        ...
        HttpContext.Current.Session["pic_code_validcode_count"] = 0; // 验证次数。3时清空Session["pic_code_validcode"]
        #endregion
